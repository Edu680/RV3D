	...
	function crearInterfaz(indicadorOperacion) {
		...
		//Campos iniciales de la interfaz
		...
		//Cambiar tamaño del escenario. Event listener.
		...
		//Lista desplegable para seleccionar operación a realizar.
	...
	//Función que genera los campos del vector resultado si la operación seleccionada es suma, resta o producto cruz.
	function generarCamposResultado(indicadorOperacion,numVectores,coordenadas) {
		if(indicadorOperacion == "Suma" || indicadorOperacion == "Resta" || indicadorOperacion == "Producto Cruz") {					
			...
			anguloRad = folderEscalar.add(text, 'anguloRad').name('Ángulo (radianes)').listen();
			folderEscalar.open();
	...
	/*Como la librería Dat.gui.js no dispone de una función para eliminar campos o apartados de la interfaz, si queremos modificar ésta según
	la operación que seleccionemos, tenemos que guardar los datos que tengamos y eliminar completamente la interfaz del DOM y volverla a crear con 
	los nuevos campos y cargando los valores que previamente guardamos*/
	...
	//El valor introducido en los campos del formulario para añadir vector se pasan a su correspondiente coordenada.
	//El color del vector creado en este módulo será aleatorio.
	...
	//Función que añade un vector al escenario
	function anadirVector() {
		
		//Se abre ventana para añadir las coordenadas del vector
		$("#crearVector").dialog({
			height:'auto', 
			width:400,
			autoOpen: true,
		...
		$("#botonAdVector").click(function() {
			...
			//Si se introducen valores no numéricos, el sistema no permite añadir vector
	...
	//Función que calcula el resultado según el tipo de operación
	...
	//Función que elimina todos los vectores del escenario y resetea la interfaz a su estado original.
